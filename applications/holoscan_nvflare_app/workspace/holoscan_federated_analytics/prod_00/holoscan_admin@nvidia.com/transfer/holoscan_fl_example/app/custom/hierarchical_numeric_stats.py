...
from hierarchical_statistics_constants import HierarchicalStatisticsConstants as HStC
            ...
            for key, value in list(structure.items()):
                ...
                if isinstance(value, list):
                    if key not in new_items:
                        new_items[HStC.GLOBAL] = get_initial_structure(value_json, ordered_metrics)
                    for i, item in enumerate(value):
                        if isinstance(item, str):
                            value[i] = {
                                HStC.NAME: item,
                                HStC.LOCAL: get_initial_structure(value_json, ordered_metrics),
                            }
                        else:
...
def get_global_stats(global_metrics: dict, client_metrics: dict, metric_task: str, hierarchy_config: dict) -> dict:
    ...
    ordered_target_metrics = HStC.ordered_statistics[metric_task]
    ...
    if HStC.GLOBAL not in global_metrics:
    ...
    for metric in ordered_metrics:
        ...
        if metric == HStC.STATS_COUNT or metric == HStC.STATS_FAILURE_COUNT or metric == HStC.STATS_SUM:
        ...
        elif metric == HStC.STATS_MAX or metric == HStC.STATS_MIN:
            for client_name in stats:
                global_metrics = get_hierarchical_mins_or_maxs(
                    metric, client_name, stats[client_name], global_metrics, hierarchy_config
                )
        elif metric == HStC.STATS_MEAN:
            global_metrics = get_hierarchical_means(metric, global_metrics)
        elif metric == HStC.STATS_HISTOGRAM:
            for client_name in stats:
                global_metrics = get_hierarchical_histograms(
                    metric, client_name, stats[client_name], global_metrics, hierarchy_config
                )
        elif metric == HStC.STATS_VAR:
            for client_name in stats:
                global_metrics = accumulate_hierarchical_metrics(
                    metric, client_name, stats[client_name], global_metrics, hierarchy_config
                )
        elif metric == HStC.STATS_STDDEV:
    ...
    def recursively_accumulate_hierarchical_metrics(
        ...
        if isinstance(global_metrics, dict):
            for key, value in global_metrics.items():
                if key == HStC.GLOBAL and HStC.NAME not in global_metrics:
                    global_metrics[HStC.GLOBAL][metric][dataset][feature] += metrics[dataset][feature]
                    continue
                if key == HStC.NAME:
                    if org and value in org:
                        ...
                        global_metrics[HStC.LOCAL][metric][dataset][feature] += metrics[dataset][feature]
                    else:
                        break
                if isinstance(value, list):
                    for item in value:
...
def get_hierarchical_mins_or_maxs(
    ...
    def recursively_update_org_mins_or_maxs(
        ...
        if isinstance(global_metrics, dict):
            for key, value in global_metrics.items():
                if key == HStC.GLOBAL and HStC.NAME not in global_metrics:
                    if global_metrics[HStC.GLOBAL][metric][dataset][feature]:
                        global_metrics[HStC.GLOBAL][metric][dataset][feature] = op(
                            global_metrics[HStC.GLOBAL][metric][dataset][feature], metrics[dataset][feature]
                        )
                    else:
                        global_metrics[HStC.GLOBAL][metric][dataset][feature] = metrics[dataset][feature]
...
def get_hierarchical_means(metric: str, global_metrics: dict) -> dict:
    ...
    def recursively_update_org_means(metrics: dict, global_metrics: dict, dataset: str, feature: str) -> dict:
        if isinstance(global_metrics, dict):
            for key, value in global_metrics.items():
                if key == HStC.GLOBAL:
                    global_metrics[HStC.GLOBAL][metric][dataset][feature] = (
                        global_metrics[HStC.GLOBAL][HStC.STATS_SUM][dataset][feature]
                        / global_metrics[HStC.GLOBAL][HStC.STATS_COUNT][dataset][feature]
                    )
                if key == HStC.LOCAL:
                    global_metrics[HStC.LOCAL][metric][dataset][feature] = (
                        global_metrics[HStC.LOCAL][HStC.STATS_SUM][dataset][feature]
                        / global_metrics[HStC.LOCAL][HStC.STATS_COUNT][dataset][feature]
                    )
                if isinstance(value, list):
                    for item in value:
    ...
    for dataset in global_metrics[HStC.GLOBAL][HStC.STATS_COUNT]:
        for feature in global_metrics[HStC.GLOBAL][HStC.STATS_COUNT][dataset]:
    ...
    def recursively_accumulate_org_histograms(
        ...
        if isinstance(global_metrics, dict):
            for key, value in global_metrics.items():
                if key == HStC.GLOBAL and HStC.NAME not in global_metrics:
                    if (
                        feature not in global_metrics[HStC.GLOBAL][metric][dataset]
                        or not global_metrics[HStC.GLOBAL][metric][dataset][feature]
                    ):
                        g_bins = []
                        ...
                        global_metrics[HStC.GLOBAL][metric][dataset][feature] = g_hist
                    else:
                        g_hist = global_metrics[HStC.GLOBAL][metric][dataset][feature]
                        ...
                        global_metrics[HStC.GLOBAL][metric][dataset][feature] = Histogram(
                ...
                if key == HStC.NAME:
                    if org and value in org:
                        ...
                        # This is a client local metrics update
                        if (
                            feature not in global_metrics[HStC.LOCAL][metric][dataset]
                            or not global_metrics[HStC.LOCAL][metric][dataset][feature]
                        ):
                            g_bins = []
                            for bucket in histogram.bins:
                                g_bins.append(Bin(bucket.low_value, bucket.high_value, bucket.sample_count))
                            g_hist = Histogram(HistogramType.STANDARD, g_bins)
                            global_metrics[HStC.LOCAL][metric][dataset][feature] = g_hist
                        else:
                            g_hist = global_metrics[HStC.LOCAL][metric][dataset][feature]
                            g_buckets = bins_to_dict(g_hist.bins)
                            for bucket in histogram.bins:
                                bin_range = BinRange(bucket.low_value, bucket.high_value)
                                if bin_range in g_buckets:
                            ...
                            for gb in g_hist.bins:
                                bin_range = BinRange(gb.low_value, gb.high_value)
                                updated_bins.append(Bin(gb.low_value, gb.high_value, g_buckets[bin_range]))
                            global_metrics[HStC.LOCAL][metric][dataset][feature] = Histogram(
                                g_hist.hist_type, updated_bins
                            )
                    else:
                        break
...
def get_hierarchical_stddevs(global_metrics: dict) -> dict:
    ...
    def recursively_update_org_stddevs(global_metrics: dict, dataset: str, feature: str) -> dict:
        if isinstance(global_metrics, dict):
            for key, value in global_metrics.items():
                if key == HStC.GLOBAL:
                    global_metrics[HStC.GLOBAL][HStC.STATS_STDDEV][dataset][feature] = sqrt(
                        global_metrics[HStC.GLOBAL][HStC.STATS_VAR][dataset][feature]
                    )
                if key == HStC.LOCAL:
                    global_metrics[HStC.LOCAL][HStC.STATS_STDDEV][dataset][feature] = sqrt(
                        global_metrics[HStC.LOCAL][HStC.STATS_VAR][dataset][feature]
                    )
                if isinstance(value, list):
                    for item in value:
                        recursively_update_org_stddevs(item, dataset, feature)

    for dataset in global_metrics[HStC.GLOBAL][HStC.STATS_VAR]:
        for feature in global_metrics[HStC.GLOBAL][HStC.STATS_VAR][dataset]:
    ...
    if isinstance(hierarchy_config, dict):
        ...
        for item in hierarchy_config:
            ...
            if isinstance(item, dict):
                result = get_client_hierarchy(item, client_name, path + [item.get(HStC.NAME)])
                if result:
                    return result
